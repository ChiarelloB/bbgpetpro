...
import { Home, PawPrint, CalendarDays, User, Plus, Radio } from 'lucide-react';
...
export const BottomNav: React.FC<BottomNavProps> = ({ activeTab, onTabChange, onAddClick }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-dark-900/90 backdrop-blur-xl border-t border-white/5 pb-8 pt-2 px-4 z-50 rounded-t-3xl">
      <div className="flex justify-between items-end">
        ...
        >
          <Home size={22} className={activeTab !== 'home' ? "group-hover:-translate-y-1 transition-transform" : ""} fill={activeTab === 'home' ? "currentColor" : "none"} />
        ...
        >
          <CalendarDays size={22} className={activeTab !== 'agenda' ? "group-hover:-translate-y-1 transition-transform" : ""} fill={activeTab === 'agenda' ? "currentColor" : "none"} />
        ...
        >
          <User size={22} className={activeTab !== 'profile' ? "group-hover:-translate-y-1 transition-transform" : ""} fill={activeTab === 'profile' ? "currentColor" : "none"} />
