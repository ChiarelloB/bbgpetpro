      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-3 gap-8 items-start">
          {testimonials.map((t, idx) => (
            <div 
              key={idx} 
              className={`bg-white dark:bg-gray-900 p-8 rounded-2xl border border-gray-100 dark:border-white/5 flex flex-col transition-all duration-300 hover:shadow-xl ${
                t.featured ? 'md:-translate-y-4 shadow-lg border-primary/20 ring-1 ring-primary/5' : 'shadow-sm'
              }`}
            >
              ...
              <div className="flex items-center gap-3">
                <div className={`size-10 rounded-full flex items-center justify-center font-black ${t.bgColor} ${t.textColor}`}>
                  {t.initials}
                </div>
                <div>
